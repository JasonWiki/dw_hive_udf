 ...
 * 这个数据是 APP 请求的时候发送的
 */
public class ParseMobileAgent extends UDF {
     /**
     * @param s sting
     * @param p pattern
     * @return
     */
    public String evaluate(String s, String p) {
        if (s == null) { return ""; }
        String base_p = p+"=([^;]+)";

        String result = "";

        String first_result = this.parseAgent(s, ";"+base_p);//先执行严格匹配，防止取p的时候把app的值取出来

        if(first_result == ""){
            String second_result = this.parseAgent(s, base_p);
    ...
    public String parseAgent(String s,String p){
        if (s == null) { return ""; }
